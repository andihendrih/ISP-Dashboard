...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
    ...
    <form method="POST" action="{{ route('genieacs.sync') }}">
        @csrf
        <button class="px-4 py-2 bg-ink text-white rounded-lg text-sm">Sync dari NBI</button>
    </form>
</div>
            ...
            @forelse($devices as $d)
                <tr class="border-t border-cream-deep/60">
                    ...
                    <td class="px-4 py-2">
                        ...
                        @elseif($d->status === 'offline') <span class="text-ink/45">offline</span>
                    ...
                    <td class="px-4 py-2 text-right space-x-2 text-xs whitespace-nowrap">
                        ...
                        <button onclick="setSsid('{{ $d->device_id }}')" class="text-ink/85 hover:underline">SSID</button>
                        <button onclick="setPwd('{{ $d->device_id }}')" class="text-ink/85 hover:underline">WiFi Pwd</button>
                    </td>
                </tr>
            @empty
                <tr><td colspan="9" class="px-4 py-8 text-center text-ink/45">Belum ada data GenieACS. Klik "Sync dari NBI".</td></tr>
            @endforelse
        </tbody>
    </table>

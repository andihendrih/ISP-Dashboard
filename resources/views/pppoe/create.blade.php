...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <form method="POST" action="{{ route('pppoe.store') }}" class="bg-white p-6 rounded-xl border border-cream-deep/60 space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <p class="text-xs text-ink/55 mt-1">Format auto: <code>{{ config('ahnet.radius.pppoe_username_prefix') }}nama#XX</code></p>
            </div>
            <div>
        ...
        <button class="px-4 py-2 bg-ink text-white rounded-lg">Simpan</button>
    </form>
    ...
    <form method="POST" action="{{ route('pppoe.bulk') }}" class="bg-white p-6 rounded-xl border border-cream-deep/60 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div>
                ...
                <input name="name_seed" required value="user" class="w-full mt-1 border border-ink/10 rounded-lg px-3 py-2">
            </div>
        </div>
        <div>
            <label class="text-sm font-medium">Group RADIUS</label>
        ...
        <button class="px-4 py-2 bg-ink text-white rounded-lg">Generate Bulk</button>
    </form>
</div>
@endsection

...
<h2 class="font-bold text-sm mb-4">Provider WhatsApp Platform</h2>
<p class="text-xs text-ink/55 mb-3">Provider WA yang lo (platform) pakai buat <strong>reminder tagihan ke tenant</strong>. Beda dari WA tenant — itu untuk tenant nagih customer-nya.</p>
<form method="POST" action="{{ route('admin.platform_settings.whatsapp.update') }}" class="space-y-3">
    @csrf
    <div>
...
<div class="mt-4 pt-4 border-t border-cream-deep/60">
    ...
    <form method="POST" action="{{ route('admin.platform_settings.whatsapp.test') }}" class="flex gap-2 flex-wrap">
        @csrf

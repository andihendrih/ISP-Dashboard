...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
    @forelse($devices as $d)
        <div class="bg-white p-5 rounded-xl border border-cream-deep/60">
            ...
            <div class="text-xs text-ink/55 font-mono">{{ $d->host }}:{{ $d->api_port }}</div>
            <div class="text-xs mt-2 space-y-0.5 text-ink/65">
    ...
    @empty
        <div class="text-ink/55">Belum ada device terdaftar.</div>
    @endforelse
</div>
...
<form method="POST" action="{{ route('mikrotik.store') }}" class="bg-white p-6 rounded-xl border border-cream-deep/60 max-w-2xl space-y-3">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <input name="username" placeholder="Username"         class="border border-ink/10 rounded-lg px-3 py-2" required>
        <input name="password" placeholder="Password"         class="border border-ink/10 rounded-lg px-3 py-2" type="password" required>
        <input name="snmp_community" placeholder="SNMP community" class="border border-ink/10 rounded-lg px-3 py-2">
    </div>
    <label class="text-sm flex items-center gap-2"><input type="checkbox" name="use_ssl" value="1"> Gunakan SSL</label>
    <button class="px-4 py-2 bg-ink text-white rounded-lg">Tambah</button>
</form>
@endsection
